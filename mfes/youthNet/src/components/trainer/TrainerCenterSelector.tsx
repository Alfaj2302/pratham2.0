...
const TrainerCenterSelector: React.FC<TrainerCenterSelectorProps> = ({
  ...
  const [centerOptions, setCenterOptions] = useState<TrainerCenter[] | null>(null);
  ...
  useEffect(() => {
    ...
    const loadStates = async () => {
      ...
      try {
        const resp = await getStateBlockDistrictList({ fieldName: 'state', sort: ['state_name', 'asc'] });
        const states = resp?.result?.values?.map((item: any) => ({
  ...
  useEffect(() => {
    ...
    const loadDistricts = async () => {
      ...
      try {
        ...
        const districts = resp?.result?.values?.map((item: any) => ({
          value: String(item.value),
          label: item.label,
        })) || [];
  ...
  useEffect(() => {
    ...
    const loadBlocks = async () => {
      ...
      try {
        ...
        const blocks = resp?.result?.values?.map((item: any) => ({
          value: String(item.value),
          label: item.label,
        })) || [];
  ...
  useEffect(() => {
    ...
    const loadVillages = async () => {
      ...
      try {
        ...
        const villages = resp?.result?.values?.map((item: any) => ({
          value: String(item.value),
          label: item.label,
        })) || [];
      ...
      <Autocomplete
        ...
        renderTags={(tagValue, getTagProps) =>
          tagValue.map((option, index) => (
            <Chip {...getTagProps({ index })} key={option.value} label={option.label} size="small" deleteIcon={<CloseIcon />} />
          ))
        }
      />
    </Grid>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 2, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            <Box sx={{ width: 32, height: 32, borderRadius: 1, bgcolor: 'warning.light', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Box>
              ...
              <Typography variant="caption" color="text.secondary">
                {hasActiveFilters ? `${activeFiltersCount} filter${activeFiltersCount > 1 ? 's' : ''} active` : 'No filters applied'}
              </Typography>
            </Box>
          </Stack>
          {hasActiveFilters && (
            <Button variant="text" size="small" startIcon={<RefreshIcon />} onClick={clearFilters} sx={{ textTransform: 'none' }}>
              Clear
            </Button>
          )}
        </Stack>
        ...
        <Grid container spacing={2}>
          {geoField('State', stateOptions, selectedState, setSelectedState, 'Select states...', loading.state, false)}
          {geoField('District', districtOptions, selectedDistrict, setSelectedDistrict, 'Select districts...', loading.district, selectedState.length === 0)}
          {geoField('Block', blockOptions, selectedBlock, setSelectedBlock, 'Select blocks...', loading.block, selectedDistrict.length === 0)}
          {geoField('Village', villageOptions, selectedVillage, setSelectedVillage, 'Select villages...', loading.village, selectedBlock.length === 0)}
        </Grid>
      </Paper>
      ...
      <Paper elevation={0} sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
        <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="caption" color="text.secondary">
              {centerOptions == null ? 'Loading…' : `${centerOptions.length} center${centerOptions.length === 1 ? '' : 's'} found`}
            </Typography>
          </Box>
        </Stack>
