...
// — no field stripping. Same fetchForm()+FormContext.mentor convention
// MentorAssignment.tsx already uses elsewhere in this app, plus the same
// "backend sometimes duplicates entries in `required`" fixup other pages
// (e.g. admin-app-repo's user-instructor.tsx) already apply.
export const getTrainerMappingForm =
  async (): Promise<TrainerMappingFormBundle | null> => {
    const fetchUrl = `${process.env.NEXT_PUBLIC_MIDDLEWARE_URL}/form/read?context=${FormContext.mentor.context}&contextType=${FormContext.mentor.contextType}`;
    ...
    // form — same fixup admin-app-repo's user-instructor.tsx already
    // applies to this same form.
    ...
    // Same 2-column grid layout admin-app-repo's user-instructor.tsx and
...
// property name. Reading it back out by L2_FIELD_IDS (the confirmed real
// Domain/Skills fieldId pair — see l2Queue.config.ts) keeps this independent
// of whatever the real property key names turn out to be.
